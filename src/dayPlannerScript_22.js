...
$addDay.addEventListener('click', e => {
    ...
    lastOption = daysSelectOptions[daysSelectOptions.length-1];
    lastDate = new Date(lastOption.value.trim());
    ...
    days.forEach(day => {
        ...
        dayActivities.forEach(activity => {
            ...
            if (lat && lng) {
                ...
                const { marker:createdMarker } = createMarker(title, {lat, lng}); // createMarker(locationInfo);   
...
function addOptionToDaysSelect2(headerText) {
    const $option = document.createElement('option');
    // $option.setAttribute('value', `day-${dayNum}`);

...
window.addEventListener('load', async () => {
    ...
    const wfLoggedInMail = localStorage.getItem('user-email');  
...
async function retrieveSavedMarkersFromFirebase2(userMail) {
    ...
    const $tripDropdownLinkClone = $tripDropdownLink.cloneNode(true);
    ...
    console.log('tripNames', tripNames);
    ...
    const querySnapshot = await getDocs(collection(db, 'travelData', `user-${userMail}`, dbCol));
    let obj;

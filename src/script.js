...
const baseUrl = 'http://localhost:3000/'; // 'https://run-gemini.onrender.com/';
...
document.querySelectorAll('.gp-first-timer-wrap input[type=radio]').forEach(btn => {
...
document.querySelectorAll('.gp-travel-with-wrap input[type=radio]').forEach(btn => {
    btn.addEventListener('click', e => {
...
document.querySelectorAll('.gp-occasion-wrap input[type=radio]').forEach(btn => {
    btn.addEventListener('click', e => {
        if (btn.value.trim()) {
...
$gpTryOutPromptBtn.addEventListener('click', async () => {

...
window.addEventListener('load', async () => {
    ...
    if (userMail) {
        await retrieveSavedMarkersFromFirebase2(userMail); 
    }

    addInLocationsFromFreeDayPlanner(); 
});

function addInLocationsFromFreeDayPlanner() {
    if (!localStorage['gp-attractions']) return; 
    JSON.parse(localStorage['gp-attractions']).forEach(attraction => {
        codeAddress(`${attraction} New York City`);

        const $allEvents = $dayEvents.querySelector('.day-event:not(.hidden) .all-events'); 
        if (!$allEvents) return; 

        const $dayEvent = $dayEvents.querySelector(`.day-event > .single-event[clonable=true]`).cloneNode(true);      
        
        $dayEvent.classList.remove('hide');
        $dayEvent.removeAttribute('clonable'); 
        // $dayEvent.id = eventId;
        $dayEvent.querySelector('.remove-marker')?.classList.remove('hide'); 
        $dayEvent.querySelector('.get-directions')?.classList.remove('hide'); 
        $dayEvent.querySelector('.day-text').textContent = format(attraction);
        ...
        const $firstLocation = $allEvents.querySelector('.single-event');
        $allEvents.insertBefore($dayEvent, $firstLocation); 
    });
}

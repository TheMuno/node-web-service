...
async function handleSubmit() {
    ...
    for(let [day, timeslots] of Object.entries(resultObj)) {
        ...
        markerCoords.push(morning?.coordinates.lat || newYorkCoords, afternoon?.coordinates.lat || newYorkCoords, evening?.coordinates.lat || newYorkCoords);
...
async function initMap(markerCoords=[]) {
    ...
    markerCoords.forEach(pair => {
        const { lat, lng } = pair;
        if (lat === 40.706005 && lng === -74.008827) return;  // skip nyc center coords

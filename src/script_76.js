...
async function handleSubmit() {
    ...
    function preprocess() {
        ...
        if (!$tripName.value.trim() || !$travelDates.value.trim()) return;
...
async function handleSaveToFirebase(userMail) {
    await createUserNSaveMarkersToDB();

    console.log('prompts', prompts)
    const prompt = generatePromptSaveString();
    await updateDoc(userRef, { prompts: arrayUnion(prompt) });
    
    async function createUserNSaveMarkersToDB() {
        let markers = localStorage['gp-markers'];
        if (!markers) return;

        await createUserInFirebase( userMail );

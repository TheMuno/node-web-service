...
console.log('currentDay on load:::', currentDay)
console.log('$daysSelect.options', $daysSelect.options)
console.log('$daysSelect.options.length', $daysSelect.options.length)
...
!function initMap() {
    ...
    searchBox.addListener('places_changed', () => { 
        ...
        places.forEach((place) => {
            ...
            const currentDay = $daysSelect.options[ $daysSelect.selectedIndex ];
            currentDay.markers = [];
...
$addDay.addEventListener('click', e => {
    ...
    currentDay = addOptionToDaysSelect2(newDate); 
    currentDay.markers = []; 
    $address.value = '';  
...
async function retrieveSavedMarkersFromFirebase2(userMail) {
    ...
    days.forEach(day => {
        ...
        dayActivities.forEach(activity => {
            ...
            if (lat && lng) {
                ...
                const { marker:createdMarker } = createMarker(dayEventName, {lat, lng}); // createMarker(locationInfo);   

...
function generatePrompt() {
    ...
    const prompt = `Create a ${days} ${dayCount} day plan for ${firstTime}${city}${travelWith}${occasion} that includes ${hotel}${attractions ? ', '+attractions : ''}${otherAttractions ? ', '+otherAttractions : ''} and restaurants. 
...
async function handleSubmit() {
    ...
    try {
        ...
        const res = `
                    <div><b>Failed to fetch, please try again!</b></div>
                    <div>${e.message}</div>
                    `;
        populateResults(res); 
    }

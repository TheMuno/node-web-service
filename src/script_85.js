...
function generatePrompt() {
    ...
    const savePrompt = `Create a <b>${dayCount} day (<i>${arrival} to ${depart}</i>)</b> day plan for <b>${firstTime}${city}${travelWith}${occasion}</b> that includes <b>${hotel}${attractions ? ', '+attractions : ''}${otherAttractions ? ', '+otherAttractions : ''}</b> and restaurants. 
    Make it sound like a story.
    Pay attention to the dates to determine the season and recommend appropriate activities.
    When you do not have a specific recommendation say "ask a local".`;
...
async function handleSubmit() {
    ...
    for(let [dayDate, timeslots] of Object.entries(resultObj)) {
        ...
        markerCoords.push(morning.coordinates, afternoon.coordinates, evening.coordinates);

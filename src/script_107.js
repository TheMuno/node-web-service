...
// (from ${days}) 
// 'Day, Month Date Year': {
    const prompt = `Create a ${dayCount} day plan (from ${days}) for ${firstTime}${city}${travelWith}${occasion} that includes ${hotel}${attractions ? ', '+attractions : ''}${otherAttractions ? ', '+otherAttractions : ''} and restaurants. 
...
async function handleSubmit() {
    ...
    function getDayPlanHTMLStr(markerInfoObj) {
        let { dayDate, morning, afternoon, evening } = markerInfoObj; 

        console.log('markerInfoObj:::', markerInfoObj)

        console.log(`dayDate.trim().startsWith('"')`, dayDate.trim().startsWith('"'))

        if (dayDate.trim() && (dayDate.trim().startsWith('"') || dayDate.trim().startsWith("'"))) {
            dayDate = dayDate.replaceAll('"','').replaceAll("'","");
        }

...
}(localStorage.getItem('gp-userMail')); 
...
$userMail.addEventListener('change', e => {
    localStorage['gp-userMail'] = e.currentTarget.value; 
});
...
async function handleSubmit(e) {
    ...
    if (!localStorage['gp-userMail']) return; 
...
async function initMap(markers) {
    ...
    //     strictBounds: false,
    //   };
    // const autocomplete = new google.maps.places.Autocomplete($hotelInp, mapOptions);

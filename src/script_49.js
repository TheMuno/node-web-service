...
!async function x() {
    const userMail = 'muijemuije24@gmail.com';

    const col = await getDocs(collection(db, 'travelData', `user-muijemuije24@gmail.com`, 'tripNYC'));
    console.log('col::::', col);

    const data = doc(db, 'travelData', `user-${userMail}`); 
    console.log('data:::', data)

    const data2 = doc(db, 'travelData', `user-${userMail}`, 'tripNYC', '2lgir3Qmt4soIhfpzErl'); 
...
async function saveMarkersToFirebase(userMail, markers) {
    ...
    */
    ...
    const data = doc(db, 'travelData', `user-${userMail}`, 'tripNYC', 'doc1'); 
    ...
    await updateCollection(data, saveObj);
}

...
!function initMap() {
    ...
    searchBox.addListener('places_changed', () => { 
        ...
        places.forEach((place) => {
            ...
            if (currentNumOfAddedActivities < 5) {
                ...
                setTimeout(()=> $form.classList.remove('shade') ,2000);
            }
    ...
    JSON.parse(localStorage['gp-attractions']).forEach(attraction => {
        ...
        const $allEvents = $dayEvents.querySelector('.day-event .all-events'); 
        const $dayEvent = $dayEvents.querySelector(`.day-event > .single-event[clonable=true]`).cloneNode(true);      
        
        console.log('$allEvents', $allEvents)
        console.log('Clonable', $dayEvents.querySelector(`.day-event > .single-event[clonable=true]`))
...
async function retrieveSavedMarkersFromFirebase2(userMail, changeTrip=false) {
    ...
    if (changeTrip) {
        ...
        if ($tripDropdown) currentTrip = await populateTripsDropdown();
    }
